...
stimFileDir = r"\\allen\programs\braintv\workgroups\nc-ophys\corbettb\motion stimuli\checkerboard stimfiles"
...
with open(r"\\allen\programs\braintv\workgroups\nc-ophys\corbettb\motion stimuli\ds_dict_checkerboard.pkl", 'rb') as f:
    ds_dict = pickle.load(f)
    

movielist = []
for i, path in enumerate(movie_path_list):
    key = int(os.path.basename(path).split('_')[0][5:])
    ds = ds_dict[key]
